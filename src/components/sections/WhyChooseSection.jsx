import { Headphones, Lock, ShieldCheck, Sparkles } from 'lucide-react'

const items = [
  {
    icon: Sparkles,
    title: 'Easy booking flow',
    text: 'Streamlined search with curated itineraries ready for expert tailoring.',
  },
  {
    icon: Headphones,
    title: 'Expert guidance',
    text: 'Speak with seasoned consultants via phone or WhatsApp in minutes.',
  },
  {
    icon: ShieldCheck,
    title: 'Price promise mindset',
    text: 'Spotted a better fare elsewhere? We work hard to match and reward loyalty.',
  },
  {
    icon: Lock,
    title: 'Secure payments',
    text: 'Bank-grade encryption and transparent instalments where suppliers allow.',
  },
]

export function WhyChooseSection() {
  return (
    <section className="border-b border-border bg-card py-16 sm:py-24">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="max-w-xl space-y-3">
          <p className="text-xs font-semibold uppercase tracking-[0.22em] text-primary">
            Why EPIC HOLIDAYS
          </p>
          <h2 className="font-display text-3xl text-foreground sm:text-4xl">
            Boutique service meets sharp airfare sourcing
          </h2>
        </div>
        <div className="mt-12 grid gap-6 md:grid-cols-2 lg:grid-cols-4">
          {items.map((item) => (
            <div
              key={item.title}
              className="rounded-[var(--radius-lg)] border border-border bg-background p-6 shadow-sm transition hover:-translate-y-1 hover:shadow-[var(--shadow-card)]"
            >
              <item.icon className="h-8 w-8 text-primary" />
              <h3 className="mt-4 font-display text-xl">{item.title}</h3>
              <p className="mt-2 text-sm text-muted">{item.text}</p>
            </div>
          ))}
        </div>
      </div>
    </section>
  )
}
